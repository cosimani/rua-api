...
@ddjj_router.get("/publico/{login}/subregistros", response_model=dict, dependencies=[Depends(verify_api_key)])
def get_subregistros_definitivos_publicos(login: str, db: Session = Depends(get_db)):
    """
    Devuelve todos los campos de subregistros definitivos de la DDJJ,
    sin requerir autenticación del usuario y sin verificar si la DDJJ está firmada.
    """
    try:
        ddjj = db.query(DDJJ).filter(DDJJ.login == login).first()

        if not ddjj:
            raise HTTPException(status_code=404, detail="No se encontró la DDJJ para el usuario indicado.")

        subregistros = {
            k: v
            for k, v in ddjj.__dict__.items()
            if k.startswith("subreg_")
        }
...
def reabrir_ddjj(
    ...
    try:
        ...
        ############# Se comenta esto para que no se notifique nada al usuario.
...
@ddjj_router.post("/reconfirmar-subregistros")
async def reconfirmar_subregistros(
    request: Request,
    db: Session = Depends(get_db),
    _: bool = Depends(verify_api_key),  # valida la api-key
):
    """
    Actualiza los subregistros de una DDJJ a partir del login y switches.
    Este endpoint es público con API-Key, y no firma ni bloquea el formulario.
    """
    ...
    if not ddjj:
        return {
            "success": False,
            "tipo_mensaje": "rojo",
            "mensaje": "No se encontró la DDJJ correspondiente.",
            "tiempo_mensaje": 6,
            "next_page": "actual"
        }

    # Lista completa de campos subregistros que querés actualizar
    subreg_keys = [
        "subreg_1", "subreg_2", "subreg_3", "subreg_4",
        "subreg_FE1", "subreg_FE2", "subreg_FE3", "subreg_FE4", "subreg_FET",
        "subreg_5A1E1", "subreg_5A1E2", "subreg_5A1E3", "subreg_5A1E4", "subreg_5A1ET",
    ...
    return {
        "success": True,
        "tipo_mensaje": "verde",
        "mensaje": "Subregistros actualizados correctamente.",
        "tiempo_mensaje": 4,
        "next_page": "",
    }